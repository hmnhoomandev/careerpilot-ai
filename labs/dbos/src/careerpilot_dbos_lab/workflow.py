"""DBOS implementation of the bounded durable-effect comparison scenario."""
...
class EffectLedger:
    ...
    def apply(self, command: PreparationCommand) -> PreparationResult:
        """Apply once per idempotency key, even when the caller retries."""
...
def configure_ledger(ledger: EffectLedger) -> None:
    """Install a synthetic ledger before launching a local test instance."""
    global _ledger
    _ledger = ledger


@DBOS.step(
    name="record_preparation_effect",
    retries_allowed=True,
    interval_seconds=0.01,
    max_attempts=2,
)
def record_preparation_effect(command: PreparationCommand) -> PreparationResult:
    """Execute the retryable, externally idempotent effect boundary."""
    return _ledger.apply(command)


@DBOS.workflow(name="prepare_application_comparison")
def prepare_application(command: PreparationCommand) -> PreparationResult:
    """Checkpoint the bounded workflow and return its durable step result."""
    return record_preparation_effect(command)
